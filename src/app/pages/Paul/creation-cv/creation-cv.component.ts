...
import {_HTTP, _HTTP_PHOTO} from "../../../CONSTANTES";
...
export class CreationCVComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.CreatEtudantCV();
    this.BindCrosImage('userPhoto', this.user.users.photo);
  }
  ...
  Add(item){
    ...
    if (item === 'FORMATION' || item === 'CERTIFICAT' || item === 'EXPERIENCE_PRO') {
      this.showNiveau = false;
  ...
  Edit(itemData, typ){
    ...
    this.initForm();
    if (itemData.hasOwnProperty('PROFIL')){
      this.showDateDebut = false;
      this.showDateFin = false;
      this.showLebelle = false;
      this.showNiveau = false;
    ...
    if (itemData.type === 'FORMATION' || itemData.type === 'CERTIFICAT' || itemData.type === 'EXPERIENCE_PRO') {
      this.showNiveau = false;
  ...
  getCvOfEtudiant(){
    this.cvService.getEtudiantCV(this.etudiant.users.etudiant.id).subscribe(creatRes => {
      this.spinner1 = false;
  ...
  DeleteElement(Data, typ){
    ...
    Swal.fire({
      title: '',
      html: "êtes-vous sûr de vouloir supprimer <br> <strong>"+Data.type+"</strong> <br> - "+Data.libelle+" <br> -"+Data.contenu,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        this.cvService.DeleteElemnt(Data.id).subscribe( deleteElemnetResponse =>{
